...
+
image::03-datascience-project.png[Composer AI Apps Project]
...
+
image::03-deploy-model-configs.png[Model Options]
change me
...
After creating the model serving instance, a ```vllm-predictor-00001-deployment-*``` pod will be spawned on the cluster in the ```vllm``` project:
[source]
oc get pods -n vllm
NAME                                              READY   STATUS            RESTARTS   AGE
vllm-predictor-00001-deployment-5cd766d5f-t52qm   0/4     PodInitializing   0          43s
...
At this point, we need to add a second GPU to the pod to have access to more memory. From the ```Models``` tab, click on the three-dots menu on the right and then edit:
...
This will create an updated deployment called ```vllm-predictor-00002-deployment-*```: 
```
oc get pods -n vllm
NAME                                               READY   STATUS    RESTARTS   AGE
vllm-predictor-00002-deployment-6f7cdc67bd-8lhkn   3/4     Running   0          17s

oc logs -n vllm -f vllm-predictor-00002-deployment-6f7cdc67bd-8lhkn 
