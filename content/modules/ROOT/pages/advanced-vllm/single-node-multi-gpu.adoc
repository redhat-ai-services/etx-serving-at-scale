...
+
image::advanced-vllm/datascience-project.png[vLLM Project]
...
+
image::advanced-vllm/single-model-serving.png[Single Model]

. Select `Deploy model`

+
image::advanced-vllm/deploy-model.png[Deploy Models]
...
+
image::advanced-vllm/deploy-model-single-gpu.png[Model Options]
...
+
image::advanced-vllm/deploy-model-data-connection.png[URI Connection]
...
+
You will find that the pod will eventually fail with the following error message:
...
+
image::advanced-vllm/edit-model-config.png[Edit configuration]

. Update `Number of accelerators` to 2: 

+
image::advanced-vllm/accelerators.png[Accelerator count]
...
+
image::advanced-vllm/deploy-model-parameters.png[Runtime Arguments]
...
+
image::advanced-vllm/ready-model.png[Dashboard Model Ready]
...
+
image::advanced-vllm/kserve-objects.png[KServe Objects]
...
+
Additionally you will see two `python` processes running, one for each GPU.
...
+
image::advanced-vllm/deploy-model-route.png[Model endpoint]
...
+
image::advanced-vllm/model-docs-page.png[Docs page]
...
+
image::advanced-vllm/llm-docs-run-v1-models.png[V1 Modules]

. Find the model name in the response body in the `id` field of the first entry in the `data` array.

+
image::advanced-vllm/model-response.png[Models Response]
...
+
image::advanced-vllm/llm-docs-strawberry.png[Chat completion]

+
Click `Execute` to send the request to the model server.  You should receive a response with an answer to the question.

+
image::advanced-vllm/completion-response.png[Completion Response]
